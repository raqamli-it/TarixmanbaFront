...
export default function Shablon() {
  ...
  const fileSize = 1024; const [progress, setProgress] = useState(0);
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const fullUrl = `${endpoints.categoryResourceApiById(`${route?.id}/`)}?${queryParams.toString()}`;
  ...
  const renderPagination = () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      if (
        ...
        pages.push(<button key={`dots-${i}`} className="px-2 py-1 mx-1 rounded bg-gray-200">...</button>);
      }
    }
  ...
  return (
    <>
      ...
      <div className="shablon-container bg-[#202020]">
        ...
        <div className="filt-cont-shablonmanba  min-w-[30%] max-w-[35%]">
          <div className="sidebar-filter min-w-[60%] max-w-[100%] ">
            <div className="bar-filter1 max-h-[100vh] min-w-[85%] min-h-[50vh] pb-[40px]   overflow-auto">
              {apiData?.category ? <h4 className="p-[20px] flex justify-center text-[30px] ">{apiData?.category}</h4> : <SkletonFilter />}
              {apiData?.period_filters?.length > 0 ?
                <div className="transition-all duration-500 px-[40px]">
                  <button
                    onClick={() => setOchil(!ochil)}
                    className={ochil ? " w-[100%] text-[20px] px-[15px] bg-[#40403F] rounded hover:bg-[#191a19] transition-all duration-500 py-[15px]" : " w-[100%] text-[20px] px-[10px] hover:bg-[#1E201E] transition-all duration-500 py-[15px]"} style={{ textAlign: "left", cursor: "pointer", position: "relative" }}
                  >
                    Davr bo'yicha
                    <span
                      style={{
                    ...
                    {ochil && (
                      ...
                      >
                        {apiData?.period_filters?.map((period) => (
                          ...
                          >
                            <span className="truncate w-[60%]">{period?.title}</span>
                            <input
                              type="checkbox"
                              className="w-[20px]"
                ...
                {apiData?.filter_categories?.map((category) => (
                  <div key={category.id} className="px-[40px]">
                    <button
                      ...
                      className={openCategory === category.id ? " w-[100%] text-[20px] px-[15px] bg-[#40403F] rounded hover:bg-[#191a19] transition-all duration-500 py-[15px]" : " w-[100%] text-[20px] rounded px-[10px] hover:bg-[#1E201E] transition-all duration-500 py-[15px]"}
                    ...
                    <AnimatePresence>
                      {openCategory === category.id && (
                        ...
                        >
                          {apiData?.filters
                            ?.filter((filter) => filter.filter_category === category.id)
                            .map((filter) => (
                              <li
                                key={filter.id}
        ...
        <div className="card-container-shablon   min-w-[65%] max-w-[65%] ">
          ...
          {loading ? ( // Display loader if loading
            ...
            apiData?.resources?.results.map((e, i) => (
              ...
              //   </div>
              // </div>
              <div className="w-[85%] flex items-center gap-1 h-[300px]" key={e?.id}>
                {/* img cont */}
                ...
                <div className="w-[67%] h-[95%] justify-between  p-[10px] flex items-center flex-col ">
                  ...
                  <div className="flex gap-[20px] items-center flex-wrap w-[100%]">
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      disabled={e?.audios && e?.audios?.length > 0 ? false : true}
                      onClick={() => document.getElementById(`audio${i}`).showModal()}
                      style={{ color: e?.audios && e?.audios?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.audios && e?.audios?.length > 0 ? "pointer" : 'not-allowed' }}
                    >Eshtuv <PiFileAudioFill className="text-[20px]" /> </button>
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      onClick={() => document.getElementById(`rasm${i}`).showModal()}
                      style={{ color: e?.galleries && e?.galleries?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.galleries && e?.galleries?.length > 0 ? "pointer" : 'not-allowed' }}
                      disabled={e?.galleries && e?.galleries?.length > 0 ? false : true}
                    >Sur'at <FaImages className="text-[20px]" /> </button>
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      onClick={() => document.getElementById(`text${i}`).showModal()}
                      style={{ color: e?.files && e?.files?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.files && e?.files?.length > 0 ? "pointer" : 'not-allowed' }}
                      disabled={e?.files && e?.files?.length > 0 ? false : true}
                    >Matn  <GrTextWrap className="text-[20px]" /></button>
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      onClick={() => document.getElementById(`map${i}`).showModal()}
                      style={{ color: e?.locations && e?.locations?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.locations && e?.locations?.length > 0 ? "pointer" : 'not-allowed' }}
                      disabled={e?.locations && e?.locations?.length > 0 ? false : true}
                    >Xarita <FaGlobeAmericas className="text-[20px]" /></button>
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      onClick={() => document.getElementById(`modul${i}`).showModal()}
                      style={{ color: e?.virtual_realities && e?.virtual_realities?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.virtual_realities && e?.virtual_realities?.length > 0 ? "pointer" : 'not-allowed' }}
                      disabled={e?.virtual_realities && e?.virtual_realities?.length > 0 ? false : true}
                    >3D <LuRotate3D className="text-[20px]" />  </button>
                    <button className="flex gap-[10px] items-center  cursor-pointer  text-white"
                      onClick={() => document.getElementById(`videos${i}`).showModal()}
                      style={{ color: e?.videos && e?.videos?.length > 0 ? "#fff" : '#a9a7a7', cursor: e?.videos && e?.videos?.length > 0 ? "pointer" : 'not-allowed' }}
                      disabled={e?.videos && e?.videos?.length > 0 ? false : true}
                    >Ko'ruv <BiSolidVideos className="text-[20px]" /> </button>

                  </div>
                  <dialog id={`audio${i}`} className="modal w-full bg-[#000000aa] h-full m-auto  ">
                    ...
                    <div className="w-full h-[90%] flex items-center " >
                      <div className="w-[500px] mx-auto p-6 bg-white rounded-lg shadow-lg">
                        {e?.audios?.map((n, i) =>
                          <AudioPlayer src={n?.audio} title={n?.title} key={n?.id || i + 1} />
                        )}
                      </div>
                    </div>
                  </dialog>
                  <dialog id={`rasm${i}`} className="modal w-full  bg-[#000000aa] h-full m-auto  ">
                    <form method="dialog" className=" w-[100%] flex justify-end">
                      <button className="btn text-[30px] text-[#fff]"><IoCloseOutline /></button>
                    </form>
                    <Swiper className="mySwiper h-[80vh]">
                      {e?.galleries?.map((m) =>
                        <SwiperSlide key={m?.id} className="text-white bg-no-repeat bg-contain bg-center " style={{ backgroundImage: `url(${m?.image})` }} ></SwiperSlide>
                      )}
                    </Swiper>
                  </dialog>
                  <dialog id={`videos${i}`} className="modal w-full bg-[#000000aa] h-full m-auto  ">
                    <form method="dialog" className=" w-[100%] flex justify-end">
                  ...
                  <dialog id={`map${i}`} className="bg-[#000000aa] w-full h-[100vh] m-auto">
                    <form method="dialog" className=" w-[100%] flex justify-end">
                  ...
                  <dialog id={`modul${i}`} className="bg-[#000000aa] w-full h-[100vh] m-auto">
                    <form method="dialog" className=" w-[100%] flex justify-end">
                      {/* if there is a button in form, it will close the modal */}
                      <button className="btn text-[30px] text-[#fff]"><IoCloseOutline /></button>
                    </form>
                    <div
                      ...
                      <div className="relative  flex flex-col gap-[20px] items-center">
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          className={`relative px-6 py-3 font-bold text-white bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 rounded-full shadow-lg transition-transform transform ${isLoading ? 'animate-pulse' : ''}`}
                        >
                          {isLoading ? (
                            <>
                  ...
                  <div className="w-[100%] flex justify-end text-white ">
                    <div className="w-[20%] flex gap-[10px] ">saqlash <FaStar className="text-[20px]" /></div>
                  </div>
            ...
            : <div className="flex flex-col sticky top-[20px] justify-center items-center bg-[#1E201E] h-[80vh] w-[100%] "><img className="w-[150px]" src="https://icons.iconarchive.com/icons/pelfusion/flat-folder/128/Cross-Folder-icon.png" alt="" />
