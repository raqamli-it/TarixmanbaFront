...
export default function Shablon() {
  ...
  const { id } = useParams(); // Params ichidan 'id' ni olish
  const searchInputRef = useRef(null);

  useEffect(() => {
    if (id) {
      const parsedId = parseInt(id, 10); // ID raqam bo'lsa, uni numberga aylantirish
      setRouteId(parsedId);
      console.log(`Hozirgi sahifa ID: ${parsedId}`);
    }
  }, [id]);
